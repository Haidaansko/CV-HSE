...
for filename in iglob(input_dir + '/*.png'):
    ...
    if mode == 'base':
        for orientation in ('horizontal', 'vertical'):
            ...
            tmp1 = seam_coords(seam_carve(img, orientation + ' shrink')[2])
            odds = []
            for i in range(len(tmp)):
                if tmp[i][1] != tmp1[i][1]:
                    odds.append((tmp[i][0], tmp[i][1], tmp1[i][1]))
            TP += tmp == tmp1
    elif mode == 'full':
        for m in (None, msk):
            for direction in ('shrink', 'expand'):
                for orientation in ('horizontal', 'vertical'):
                    tmp = load(file)
                    tmp1 = seam_coords(seam_carve(img, orientation + ' ' + direction, mask = m)[2])
                    b = tmp == tmp1
                    TP += b
